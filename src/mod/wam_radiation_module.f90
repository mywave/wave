...
&                        * TFAK(INDEP(NIJS:NIJL),M) * DFIMOFR(M)/ZPI
   ...
   CALL PUT_DRY(SXX, NIJS, NIJL, ZMISS)   !! FLAG DRY POINTS
   CALL PUT_DRY(SYY, NIJS, NIJL, ZMISS)
   CALL PUT_DRY(SXY, NIJS, NIJL, ZMISS)
   CALL PUT_ICE(SXX, NIJS, NIJL, ZMISS)   !! FLAG ICE POINTS
   CALL PUT_ICE(SYY, NIJS, NIJL, ZMISS)
   CALL PUT_ICE(SXY, NIJS, NIJL, ZMISS)
...
IF (PRESENT(D_LON)) THEN
   ...
   DO IJ = nijs,nijl
      ...
      ELSE IF (IJ2.GT.ninf-1) THEN
         ...
         END IF
      END IF
      IF (SPHERICAL_RUN) D_LON(IJ) = D_LON(IJ)*DCO(IJ)
